...
class CreateAppointmentViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            cell.profilePictureImageView.image = patient.profilePicture
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 3 {
            ...
            appointment = Appointment(startDate: date, endDate: date.addingTimeInterval(TimeInterval(60.0 * 60)), doctorUid: doctorUid, patientUid: patient.uid)
            ...
            if !calendarIsBusy(appointment: appointment!, forPerson: doctor as AnyObject) {
                ...
                if !calendarIsBusy(appointment: appointment!, forPerson: patient as AnyObject) {
                    ...
                    if let errorMessage = appointment?.createInCalendar(patient: patient) {
                        SCLAlertView().showError(NSLocalizedString("appointment.create.errror", comment: ""), subTitle: errorMessage)
                    } else {
                        SCLAlertView().showSuccess(NSLocalizedString("appointment.create.sucess.title", comment: ""), subTitle: NSLocalizedString("appointment.create.sucess.message", comment: ""))
                        dismiss(animated: true, completion: nil)
                    }
                } else {
                    SCLAlertView().showNotice(NSLocalizedString("appointment.create.error.busy.title", comment: ""), subTitle: NSLocalizedString("appointment.create.error.busy.message", comment: ""))
                }
            } else {
                SCLAlertView().showNotice(NSLocalizedString("appointment.create.error.busy.title", comment: ""), subTitle: NSLocalizedString("appointment.create.error.Doctorbusy.message" , comment: ""))
            }
        }
    }
    ...
    @IBAction func pickerView(_ sender: UIDatePicker) {
        ...
        if components.hour! < 7 {
            components.hour = 7
            components.minute = 0
            SCLAlertView().showWarning(NSLocalizedString("appointment.create.timeError.title", comment: ""), subTitle: NSLocalizedString("appointment.create.earlyError.message", comment: ""))
            sender.setDate(Calendar.current.date(from: components)!, animated: true)
        } else if components.hour! > 21 {
            components.hour = 21
            components.minute = 59
            SCLAlertView().showWarning(NSLocalizedString("appointment.create.lateError.title", comment: ""), subTitle: NSLocalizedString("appointment.create.timeError.message", comment: ""))
            sender.setDate(Calendar.current.date(from: components)!, animated: true)
        }
    }
